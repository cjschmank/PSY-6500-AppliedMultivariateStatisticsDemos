...
#Breusch-Pagan test 
bptest(bplm, varformula = ~Belief+Need+Interest+Happy, data=dat_final[2:6],studentize=FALSE)
bptest(bplm,varformula = ~Belief+Need+Interest+Happy, data=dat_final[2:6], studentize=TRUE)
...
```{r}
set.seed(13)
apa.reg.boot.table(lm2,
                   number.samples = 1000)

apa.cor.table(dat_final[2:6])
```
...
ggplot(data = dat_final,
       mapping = aes(x = predictedF, y = residuals)) +
  geom_smooth(method = "lm", formula = y ~ x, se = FALSE) +
  geom_point() +
  labs(x = "Predicted DV", y = "Residuals")


ggplot(data = dat_final,
       mapping = aes(x = residuals)) +
  geom_histogram(bins = 12, color = "goldenrod3")

describe(dat_final$residuals)
```
