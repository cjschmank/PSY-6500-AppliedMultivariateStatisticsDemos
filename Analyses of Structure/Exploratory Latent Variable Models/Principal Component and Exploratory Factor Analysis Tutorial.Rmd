...
# Always visualize your correlation matrix object in case of NAs
wisc_corr
```
...
cortest.bartlett(wisc_dat)

# Null hypothesis is that the data is an identity matrix!
```
...
knitr::include_graphics("Figures/KMOMetrics.png")
```
...
print.psych(wisc_pca, sort = TRUE)
# print.psych(wisc_pca, cut=.3,sort = TRUE)

# Before parallel analysis we could use the SS loadings (i.e., Eigenvalues) to assess the number of components that should be extracted--based on Kaiser cutoff values of 1.00 for Eigenvalues we could have 3 components based on this correlation matrix NOT 2
...
Now the *h^2^* communality values are more "interesting" when we extract FEWER components than items (not all 11). For instance, 37% of the variance in `parang`is captured by the three components (meaning 63% is left unaccounted for!). But if we look at the factor loadings for `parang` in the rotated model, we have a much easier time determining which component explains it. **Remember**: This is just an artifact of running PCA with fewer components than variables, PCA ALWAYS ACCOUNTS FOR 100% OF THE VARIANCE! 
...
# ML Extraction
resid1_upper <- as.matrix(resid1[upper.tri(resid1)])
resid1_upper
mean(resid1_upper)
```
...
```{r}
# PAF Extraction
resid2_upper <- as.matrix(resid2[upper.tri(resid2)])
resid2_upper
mean(resid2_upper)
```
...
```{r}
# GLS Extraction
resid3_upper <- as.matrix(resid3[upper.tri(resid3)])
resid3_upper
mean(resid3_upper)
```
...
```{r}
EGA(wisc_dat_2)
groups <- list(Verbal = c(1:6), Performance = c(7:10))
network <- qgraph::EBICglasso(wisc_cor2, n = 175)
qgraph::qgraph(network, layout = "circle", groups = groups, palette= "colorblind")
qgraph::qgraph(network, layout = "spring", groups = groups, theme= "gray")
```
